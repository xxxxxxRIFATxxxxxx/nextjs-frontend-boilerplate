...
const UserBlogCategoryList = ({ initialBlogCategories }) => {
    ...
    const goToItemDetails = (item) => {
        router.push(`/blog-categories/${item?.slug}`);
    };
    ...
    useEffect(() => {
        ...
        if (search) {
            ...
            filtered = filtered.filter((item) =>
                [item?._id?.toString(), item?.title, item?.slug]
    ...
    }, [search, sortBy, startDate, endDate, blogCategories]);
        ...
        <div>
            ...
            <div>
                {filteredItems?.length === 0 ? (
                    <div>No blog categories found.</div>
                ) : (
...
export default UserBlogCategoryList;
